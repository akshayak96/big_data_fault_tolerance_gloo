...
Runner::Runner(const options& options) : options_(options) {
  ...
  if (!contextFactory_) {
    std::cout << "gloo using file system\n";
    rendezvousFileSystem();
  }

  std::cout << "trying to enforce contextFactory_\n";
  ...
  std::cout<< "finished broadcast between participants" << std::endl;
...
void Runner::rendezvousRedis() {
  ...
  //replaced redis rendezvous logic with zookeeper logic
  //TODO: define GLOO_USE_ZOOKEEPER and place zookeeper logic inside it. Haven't done it yet as it requires changes to CMakeLists.
  std::cout<<"inside zookeeper rendezvous"<<std::endl;


  //calling zookeeper constructor with the host and port information specified in options
  rendezvous::ZookeeperStore zookeeperStore(options_.redisHost, options_.redisPort);

  std::cout<<"zookeeper constructor called" <<std::endl;

  //rendezvous::RedisStore redisStore(options_.redisHost, options_.redisPort);

  std::cout<<"prefix store called with zookeeper store" <<std::endl;

  rendezvous::PrefixStore prefixStore(options_.prefix, zookeeperStore);


  std::cout<<"calling rendezvous context with zookeeper store" <<std::endl;
  auto backingContext = std::make_shared<rendezvous::Context>(
      options_.contextRank, options_.contextSize);
  std::cout<<"connecting mesh with devices" << std::endl;
